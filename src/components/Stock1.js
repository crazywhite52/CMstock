...
import soundfile from "../Facebook.mp3";
import {
  Container,
  Row,
  Col,
...
//const API = 'http://172.18.0.135:8888/api_stock';
...
class Stock1 extends React.Component {
  ...
  componentDidMount() {
    ...
    //this.intervalId = setInterval(() => this.fetchUsers(), 100000);
    //this.fetchUsers();
    // this.setState({ isLoading: true });
    // this.timeout = setTimeout(() => {
    //   fetch(API)
  ...
  response = () => {
    ...
    socket.on("StockApicommart", data => {
      ...
      //console.log(data.StockApicommart[0]);
    });
  };
  ...
  print(e, p) {
    this.setState({ print: p }, function () {
            ...
            <Table small striped hover responsive>
              <TableHead style={bgTh}>
                <tr>
                  ...
                  <th className="text-center">
                    <h4>
                      <strong style={font2}>สินค้า</strong>
                    </h4>
                  </th>
...
export default withAuth(Stock1);
